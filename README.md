...
- 导入时强制带 `--auto-login`
- 浏览器登录态过期时，自动重新登录财税通，并自动识别登录页验证码
...
- 只按最新流程：Step1 + Step2 + Step2.5 + Step3 + Step4
- `01_添加员工` 支持新列 `角色类型`、`角色名称`，会按普通角色/部门角色自动建角并绑人
- `审批流` sheet 会按单据名创建或更新审批流，并把生成的 `workflowId` 回填给单据模板
- `权限` sheet 会把权限名称与角色/员工做增量绑定
- 借款单/申请单不进入费用限制分支
- Sheet2 `归属单据名称` 与 Sheet3 `单据模板名称` 必须一一对应
- 核心 sheet 仍是 `01_添加员工`、`02_费用科目配置`、`03_单据表`，额外支持 `审批流`、`权限`
